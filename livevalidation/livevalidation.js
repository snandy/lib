// Validation 1.4 (standalone version)
// Copyright (c) 2007-2010 Alec Hill (www.livevalidation.com)
// Validation is licensed under the terms of the MIT License
...
/**
 * pass an array of Validation objects and it will validate all of them
 * @param validations {Array} - an array of Validation objects
...
Validation.prototype = {
	...
	/**
	 * adds a validation to perform to a Validation object
	 ...
	 * @return {Object} - the Validation object itself so that calls can be chained
	 */
	...
	/**
	 * removes a validation from a Validation object - must have exactly the same arguments as used to add it 
	 * @param validateFunc {Function} - validation function to be used (ie Validate.presence )
	 * @param validateOption {Object} - parameters for doing the validation, if wanted or necessary
	 * @return {Object} - the Validation object itself so that calls can be chained
	 */
	...
	/**
	 * loops through all the validations added to the Validation object and checks them one by one
	 * @param validateFunc {Function} - validation function to be used (ie Validate.presence )
	 * @param validateOption {Object} - parameters for doing the validation, if wanted or necessary
	...
	/**
	 * enables the field
	 * @return {Validation} - the Validation object for chaining
	 */
	enable: function() {
...
/**
 * This class is used internally by Validation class to associate a Validation field with a form it is icontained in one
 * 
 * It will therefore not really ever be needed to be used directly by the developer, unless they want to associate a Validation 
 * field with a form that it is not a child of, or add some extra functionality via the hooks (access through a Validation object's formObj property)
 */

/**
 * handles validation of Validation fields belonging to this form on its submittal
 * @param element {HTMLFormElement} - a dom element reference to the form to turn into a ValidationForm
 */
...
/**
   *  gets the instance of the ValidationForm if it has already been made or creates it if it doesnt exist
   *  
...
ValidationForm.prototype = {
	...
	/**
	 * constructor for ValidationForm - handles validation of Validation fields belonging to this form on its submittal
	 * @param element {HTMLFormElement} - a dom element reference to the form to turn into a ValidationForm
	 */
	...
	/**
	 * adds a Validation field to the forms fields array
	 * @param element {Validation} - a Validation object
	 */
	addField: function(newField) {
		this.fields.push(newField)
	},
	/**
	 * removes a Validation field from the forms fields array
	 * @param victim {Validation} - a Validation object
	 */
...
var Validate = {
	 ...
	 *							(DEFAULT: 'Must be {is}!')
	 *		tooLowMsg {String}	- the message to show when the validation fails when min param is used
	 ...
	 *		max {Int} - the maximum length allowed
	 ...
	 *
	 * option properties:
	 *		failureMsg {String} - the message to show when the field fails validation
	 *						(DEFAULT: 'Not valid!')
   ...
   *						(DEFAULT: false)
	 ...
	 *	If you do not want this to be the case then you must either add a Validation.presence validation
	...
	email: function(val, option) {
		...
		Validate.format(val, { failureMsg: message, pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i } )
		return true
	},
	/**
	 ...
	 *		tooShortMessage {String}  - the message to show when the fails when min param is used
	 *								(DEFAULT: 'Must not be less than {min} characters long!')
	 *		tooLongMessage {String}   - the message to show when the fails when maximum param is used
	 *								(DEFAULT: 'Must not be more than {maximum} characters long!')
	 *		is {Int}		  - the length must be this long 
	 *		min {Int}		 - the minimun length allowed
	 *		max {Int}		 - the maximum length allowed
	 *
	 *  NB. can be checked if it is within a range by specifying both a minimum and a maximum	   
	 */
